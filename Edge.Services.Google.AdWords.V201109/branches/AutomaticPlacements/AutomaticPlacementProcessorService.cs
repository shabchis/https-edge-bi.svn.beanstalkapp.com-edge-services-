			...
			using (var session = new TargetingMetricsImportManager(this.Instance.InstanceID))
			{
				...
				using (_placementReader)
				{
					while (_placementReader.Read())
					{
						...
						string adId = _placementReader.Current[Const.AdIDFieldName];
						if (!importedAds.ContainsKey(adId))
						{
							...
							//Ad Type
							string adTypeKey = Convert.ToString(_placementReader.Current[Const.AdTypeFieldName]);
							ad.ExtraFields[AdType] = GoogleAdTypeDic[adTypeKey];
							ad.Creatives.Add(new TextCreative { TextType = TextCreativeType.DisplayUrl, Text = _placementReader.Current[Const.DisplayURLFieldName] });

							////Setting Tracker for Ad
							if (!String.IsNullOrWhiteSpace(_placementReader.Current[Const.DestUrlFieldName]))
							{
								ad.DestinationUrl = _placementReader.Current[Const.DestUrlFieldName];
								SegmentValue tracker = this.AutoSegments.ExtractSegmentValue(Segment.TrackerSegment, _placementReader.Current[Const.DestUrlFieldName]);
								if (tracker != null)
