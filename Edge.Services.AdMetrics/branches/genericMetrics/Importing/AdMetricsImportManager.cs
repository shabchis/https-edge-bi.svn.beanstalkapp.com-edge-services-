...
namespace Edge.Services.AdMetrics
{
	...
	public class AdMetricsImportManager : DeliveryImportManager, IDisposable
	{
		...
		public void ImportAd(Ad ad)
		{
			...
			CampaignSegment campaign = (CampaignSegment) ad.Segments[SegmentType.Campaign];
			...
			var adRow = new Dictionary<ColumnDef, object>()
			{
				...
				{Tables.Ad.Campaign_Account_ID, campaign.Account.ID},
				{Tables.Ad.Campaign_Account_OriginalID, campaign.Account.OriginalID},
				{Tables.Ad.Campaign_Channel, campaign.Channel.ID},
				{Tables.Ad.Campaign_Name, campaign.Value},
				{Tables.Ad.Campaign_OriginalID, campaign.OriginalID},
				{Tables.Ad.Campaign_Status, campaign.Status == ObjectStatus.Unknown ? (object) DBNull.Value : (object) campaign.Status},
			};
			...
			// AdTarget
			foreach (Target target in ad.TargetingOptions)
			{
			...
			// AdSegment
			foreach (KeyValuePair<SegmentType, Segment> segment in ad.Segments)
			{
		...
		public void ImportMetrics(AdMetricsUnit metrics)
		{
			...
			string metricsGuid = metrics.Usid.ToString("N");
			...
			foreach (Target target in metrics.TargetingDimensions)
			{
				var row = new Dictionary<ColumnDef, object>()
				{
				...
				{
					foreach (KeyValuePair<SegmentType, Segment> segment in target.Segments)
					{
