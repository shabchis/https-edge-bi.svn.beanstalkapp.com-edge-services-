...
namespace Edge.Services.Google.AdWords
{
    public class RetrieverService : PipelineService
    {
        ...
        protected override Core.Services.ServiceOutcome DoPipelineWork()
        {
            ...
            _filesInProgress = Delivery.Files.Count;
            ...
            string startDate = Configuration.TimePeriod.Value.Start.ToDateTime().ToString("yyyyMMdd");
			string endDate = Configuration.TimePeriod.Value.End.ToDateTime().ToString("yyyyMMdd");
            _waitHandle = new AutoResetEvent(false);

            foreach (var clientId in (string[])Delivery.Parameters["AdwordsClientIDs"])
            {
                //Get all files on specific client
                var files = from f in Delivery.Files
